...
def disparity_assessment(ref_image, neighbor_image, disp,extra_info):
    # Warping
    root_path = "./out_images/"+extra_info[0][0]
    if not os.path.exists(root_path):
        os.makedirs(root_path)
    writeFlow(
        os.path.join(
            root_path,
            "frame"+str(extra_info[1][0])+
            ".flo"),
        disp
    )
    cv2.imwrite(
...
def demo(args,data_loader):
    ...
    for idx, sample in enumerate(data_loader):
        with torch.no_grad():
            im0, im1, extra_info = sample
            im0 = im0.to(DEVICE)
            im1 = im1.to(DEVICE)
            print(im0.shape)
            padder=InputPadder(im0.shape)
            im0,im1=padder.pad(im0,im1)
            flow_low, flow_up = model(im0, im1, iters=20, test_mode=True)
